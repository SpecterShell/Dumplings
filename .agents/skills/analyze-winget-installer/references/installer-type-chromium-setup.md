...
| --- | --- |
| `ChromiumMiniInstaller` | `B7` or `BN` resource named `chrome.packed.7z`/`chrome.7z`, plus `BL` or `BN` resource named `setup.ex_`/`setup.exe` |
...
Test-ChromiumMiniInstaller -Path $InstallerFile
Test-ChromiumUpdater -Path $InstallerFile
Test-OmahaInstaller -Path $InstallerFile
Read-ChromiumInstallerTag -Path $InstallerFile
```
