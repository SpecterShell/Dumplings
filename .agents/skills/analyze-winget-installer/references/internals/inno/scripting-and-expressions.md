...
Dumplings uses the format catalog to locate `CompiledCodeText`. It is absent from official structures before 4.0, but appears in the earlier My Inno Setup Extensions structure. Ordinary `Get-InnoInfo` validates only the bounded `IFPS` header and returns its counts. `Get-InnoInfo -IncludePascalScriptAnalysis` decodes the program during the same setup-header parse. The explicit `Get-InnoPascalScriptInfo` operation is for script-only investigation.
...
A small constant evaluator accepts only straight-line assignments and primitive operations. It rejects calls, branches, exception flow, pointers, indexed operands, and unknown opcodes. When such a function returns one string, matching `{code:Function}` header constants can be expanded without running the program. The parser records every accepted substitution in `ResolvedPascalCodeConstants`; all other code constants remain unresolved.
