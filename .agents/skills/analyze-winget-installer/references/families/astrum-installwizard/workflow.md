...
Controlled VM installations of the 2.29.50 probe matrix establish the installed-state behavior: successful silent installation writes the compiled uninstall key (32-bit or 64-bit view per the x64-compliance option) and returns exit code `1`; an as-invoker manifest with compiled HKLM writes refuses unelevated with exit code `0` and no partial state; a `requireAdministrator` manifest refuses unelevated with exit code `5`; a hidden-ARP configuration writes no uninstall key at all; and a disabled-uninstaller configuration still writes the ARP entry whose `UninstallString` points at a generated-uninstaller file that is never created.
...
2. Review `ContainerRoute`, `FormatCatalogId`, `FormatGeneration`, `ConfigurationProfile`, `ProductCode`, `DisplayName`, `DisplayVersion`, `Publisher`, `Scope`, `DefaultInstallLocation`, `RequestedExecutionLevel`, `RegistryView`, `Variables`, `Diagnostics`, and `UnresolvedFields`. `astrum-1` and `astrum-2` identify catalogued overlay layouts; `Legacy1`, `Early2`, and `Modern2` identify configuration layouts selected within them rather than inferred builder editions. `ParserVersionInfo.CatalogVersion` records the descriptor schema used for the parse. Custom-variable records expose `TypeName`, `SourceName`, `DefaultValue`, source locations, and decoded option flags. `Nowhere` values resolve from their literal defaults; the exact empty Registry/HKEY_CLASSES_ROOT fallback proven by BreakAlube also resolves. Other registry-, INI-, file-, dialog-, timer-, or DLL-backed values remain under `ArpEntries` and are omitted from manifest-facing `AppsAndFeaturesEntries` fields.

3. Inspect `RegistryWrites`, `Shortcuts`, `FileExtensionAssociations`, `ProtocolAssociations`, `IniOperations`, `TextOperations`, `FileOperations`, `InteractiveOperations`, `PostInteractiveRecords`, and `ExecutedPayloads`. Use `OperationName`, `ActionName`, `TimingName`, and condition-term `OperatorName` instead of translating numeric codes locally. `ExecutedPayloads` contains both “Execute program” and “Execute program and wait”; analyze each configured nested executable separately before composing wrapper switches or assigning ARP ownership. Astrum 1.x interactive records end after `ExecuteCount`; only 2.x records expose flags, a custom message, and a condition. `PostInteractiveRecords` is an observed 2.x table and is distinct from builder Resource files. Conditional records are evidence, not guaranteed installed state.

4. Inspect `InstallationItems`, `PayloadCatalog`, `ResourceFiles`, and `CompressionEvidence`. Builder Resource files are ordinary catalog records beginning with `<ResourceDir>\`; they are decompressed before installation and can contain prerequisites used by interactive actions. A destination beginning with `<InstallDir>` maps below the application root; other known destination roots are isolated under `_destinations` during extraction.

5. Review `Requirements`, `RuntimeCommandLineEvidence`, `LicenseDialogSelected`, `LicenseAcceptanceRequired`, `UserInformationBlocksSilent`, `SilentInstallationDefault`, `NoUninstallation`, `X64ComplianceMode`, and `RequireAdmin`. Sparse requirement and policy offsets are projected only for `Modern2`, where controlled 2.29 builds establish them. The minimum Windows-family/version, display bit depth, Java version, and .NET Framework selector are included. Fields after the variable-length Java string use its terminator as an anchor, so Java requirements cannot shift silent, uninstall, x64, or elevation flags into the wrong offsets. `Legacy1` and `Early2` retain their option blocks as bounded `OptionBlockEvidence` without applying modern offsets. `AssignedFields` identifies source-backed offsets and `UnassignedRanges` preserves opaque bytes without turning unrelated builder options into a parse warning. For 1.x, an exact `/SILENT` token delimited inside the native PE image proves that the runtime accepts the switch; `Astrum.Silent.LegacyRuntimeVersionRequired` is returned only when that token is absent.
...
8. Resolve each structured diagnostic for the current workflow. `Astrum.Silent.UserInformationDialog` proves that an artifact is interactive-only, `Astrum.Silent.UserInformationDialogIgnored` records that the compiled dialog is skipped under `/silent` for the current generation, `Astrum.Silent.LicenseAcceptance` proves that `/AcceptLicense` is required, `Astrum.Silent.LegacyRuntimeVersionRequired` requires builder-version or VM evidence before assigning a 1.x switch when runtime-table evidence is absent, `Astrum.Elevation.CallerRequired` identifies as-invoker machine media that must be launched by an elevated caller, and `Astrum.Uninstall.Disabled` means the generated uninstaller is absent even if compiled ARP writes remain. Preserve existing manifest fields when other incomplete evidence does not affect the fields being refreshed.
...
Use the compiled registry hive first, then requested execution level and destination evidence. The supplied 2.29.50 media writes HKLM ARP entries and requests administrator elevation, so it is machine scope with `ElevationRequirement: elevationRequired`. The compiled `RequireAdmin` option also proves `elevationRequired` even when the PE manifest does not request elevation. An `asInvoker` artifact that targets unconditional HKLM state or a machine-protected installation path also requires an elevated caller: controlled Modern2 `/silent` execution refuses unelevated with exit code `0` and no partial state, so the parser emits `Astrum.Elevation.CallerRequired` and projects `elevationRequired`. Mixed or conditional hive evidence must remain unresolved until validated dynamically.
...
Follow [VM validation](../../workflows/vm-validation.md). For 2.x, test the exact suggested command separately, capture exit code `1`, confirm the application files and generated uninstaller, and compare ARP, protocol, and file-extension snapshots. For 1.x, runtime-table evidence is sufficient to propose `/silent`, but test the switch and capture the process result before authoring `InstallerSuccessCodes`. Test the application after installation and inspect configured nested commands. When the parser reports `LicenseAcceptanceRequired`, validate `/silent /AcceptLicense`; when it reports `UserInformationBlocksSilent`, do not claim unattended support without contradictory artifact-specific VM evidence.

A cross-generation audit of all 46 cached artifacts, including 20 archived builder installers spanning 1.80 through 2.29.50, confirms the catalog generations, the silent-route evidence, and the decoded option values against readable `.ai2` project files. Project files that request out-of-range dropdown values compile to the builder's clamped defaults, so a decoded requirement can differ from project intent; trust the decoded compiled values over project-file wishes.

Normal Astrum 1.x and 2.x single-file media are supported. `/tiny` and `/tinyverbose` are verified for 1.95.5 and 2.29.50. Explicitly supplied spanned media, modern fixed-option projection, and Authenticode-after-payload routing are verified for 2.x. Companion files are never discovered by wildcard inside the parser: pass the complete ordered list and treat a missing or extra volume as an error.
...
- Controlled 1.95.5 and 2.29.50 tiny and tiny-verbose wrappers: one bounded GZip member containing the complete inner installer and a validated 96-byte wrapper descriptor.
- Controlled five-part spanned medium: one catalog record crossing the main EXE and four companion volumes.
- BreakAlube PC-GINA 1.0.1.5: five payload files across application, driver, and manual groups; `<TimerName>` uses the builder's `Nowhere` source, while `<MainPath>` uses the empty Registry/HKEY_CLASSES_ROOT fallback that the runtime resolves to its compiled default.
