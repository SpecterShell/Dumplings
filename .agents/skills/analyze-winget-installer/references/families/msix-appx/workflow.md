...
Read [MSIX And AppX Installer Type Parser Internals](../../internals/msix-appx/overview.md) before changing detection, extraction, binary decoding, or parser limits.

### Parse package identity, dependencies, and signature
...
### Record packaged-app identity and display evidence
...
- `Microsoft.VCLibs.14`
- `Microsoft.WindowsAppRuntime.2` (mapped to `Microsoft.WindowsAppRuntime.2.0`)
...
- `DuckDuckGo.DesktopBrowser`: packaged desktop application identity.
- `FilesCommunity.Files`: framework dependency projection.
- `TheBrowserCompany.Arc`: desktop VCLibs dependency mapping.
- `Microsoft.WindowsApp`: Windows App Runtime dependency evidence.
