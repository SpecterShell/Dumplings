...
Read [Burn Installer Type Parser Internals](../../internals/burn/overview.md) before changing detection, extraction, binary decoding, or parser limits.
...
- Visibility or conditions remain ambiguous: continue static analysis but require [VM validation](#vm-validation) before submission.
...
### Determine switches and install modes
...
- Unverified variable or package condition: do not add it from a string match alone; route to [VM validation](#vm-validation).

### Analyze chained payload metadata and associations
...
If protocol or file-extension registration is performed only by a custom bootstrapper action or first application run, static Burn metadata is insufficient; route it to [VM validation](#vm-validation).
...
Use this shape when [visible ARP analysis](#identify-the-visible-arp-owner) proves that the Burn bundle writes the visible entry, [scope analysis](#determine-scope) finds one supported scope, and no Apps & Features override is required. Obtain the bundle `ProductCode` with `Read-ProductCodeFromBurn`. Add `Scope` only when `Get-BurnScopeInfo.DefaultScope` is conclusive.
...
### Chained MSI/WiX owns visible ARP

Use this shape when [visible ARP analysis](#identify-the-visible-arp-owner) proves that the bundle registration is absent or hidden and a chained MSI/WiX package owns the visible entry. Parse that MSI with `Get-MsiInstallerInfo` and use its visible product code and upgrade code. Keep outer `InstallerType: burn` because WinGet invokes the bundle.
...
Follow [VM validation workflow](../../workflows/vm-validation.md) for conditional chain registration, bundle-versus-payload ARP ownership, scope routes, quiet/passive behavior, and outer/nested exit-code forwarding.

## Known examples

- `Python.Python.3.14`: dual-scope Burn bundle.
- `Crestron.AirMedia`: user-scope Burn bundle.
- `Sinew.Enpass`: architecture restrictions derived from bundle conditions.
- `Jabra.Direct`: machine-scope bundle with chained payload evidence.
