...
`Get-DeployMasterInfo` validates the locator, file size, CRC32, current or legacy control-header layout, and structured LZMA metadata. Reuse the returned object instead of calling multiple `Read-*FromDeployMaster` helpers:
...
An executable index of `-1` means the action did not resolve to a packaged file and should not be treated as an installed open command. Protocols and arbitrary custom registry associations still require separate static or VM evidence.
...
Follow [VM validation workflow](../../workflows/vm-validation.md) for silent behavior, exit codes, default scope of a dual-scope package, custom ARP writes, or first-run associations. DeployMaster-specific checks are:
