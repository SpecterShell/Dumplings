...
$Object2 = (Invoke-RestMethod -Uri "${Prefix}/catalog_win3_x64.xml").Catalog.CatalogVersion.Installers.Where({ $_.name -eq '1CDF566D-B2C7-47CA-802F-6283C862E1D6' -or $_.name -eq 'WorkspaceAppX64' }, 'First')[0].Installer.Where({ $_.Stream -eq 'Current' }) | Sort-Object -Property { $_.Version -replace '\d+', { $_.Value.PadLeft(20) } } -Bottom 1
