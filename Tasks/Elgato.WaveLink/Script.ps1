...
switch -Regex ($this.Check()) {
  ...
  'Updated' {
    ...
    $Mutex = [System.Threading.Mutex]::new($false, 'DumplingsSubmitLockWaveLink')
    $Mutex.WaitOne(30000) | Out-Null
    if (-not $Global:DumplingsStorage.Contains("WaveLink-$($this.CurrentState.Version)-ToSubmit")) {
      $Global:DumplingsStorage["WaveLink-$($this.CurrentState.Version)-ToSubmit"] = $ToSubmit = $true
    }
    $Mutex.ReleaseMutex()
    $Mutex.Dispose()
