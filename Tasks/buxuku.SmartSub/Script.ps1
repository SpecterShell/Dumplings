$Object1 = Invoke-GitHubApi -Uri "https://api.github.com/repos/buxuku/SmartSub/releases/latest"
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        # ReleaseNotes (en-US)
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node -and -not ($Node.Name -match 'h\d' -and $Node.InnerText -match '下载'); $Node = $Node.NextSibling) { $Node }
