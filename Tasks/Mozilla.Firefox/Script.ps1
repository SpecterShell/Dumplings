...
# Version
$OriginalVersion = $Object1.LATEST_FIREFOX_VERSION
$this.CurrentState.Version = $ShortVersion = $OriginalVersion
...
switch -Regex ($this.Check()) {
  ...
  'Updated' {
    foreach ($Locale in $Locales) {
      ...
      if ($Locale -eq 'multi') {
        $this.Config.WinGetIdentifier = 'Mozilla.Firefox.MSIX'
        ...
        if ($Locale -eq 'en-US') {
          $this.Config.WinGetIdentifier = 'Mozilla.Firefox'
        } else {
          $this.Config.WinGetIdentifier = "Mozilla.Firefox.${Locale}"
        }
