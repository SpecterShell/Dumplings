# $Object1 = Invoke-RestMethod -Uri 'https://releases.threema.ch/web-electron/v1/updates/work-latest-windows.json'
...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://threema.com/en/changelog/desktop' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//div[contains(@id, 'accordion') and contains(@id, 'heading') and contains(., 'Threema Work $($this.CurrentState.Version)')]")
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://releases.threema.ch/web-electron/v1/release/Threema-Latest.exe'
}
