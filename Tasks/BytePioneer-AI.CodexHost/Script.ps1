$Object1 = Invoke-GitHubApi -Uri 'https://api.github.com/repos/BytePioneer-AI/codex-host/releases/latest'
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node -and -not ($Node.Name -match '^h[12]$' -and $Node.InnerText -match 'Installation'); $Node = $Node.NextSibling) { $Node }
        # ReleaseNotes (zh-CN)
