...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Global:DumplingsStorage.NICESoftwareDownloadPage.Links.Where({ try { $_.href.EndsWith('.msi') -and $_.href.Contains('nice-dcv-server') } catch {} }, 'First')[0].href
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      ...
      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//tr[contains(./td[1], 'nice-dcv-server: $($this.CurrentState.Version.Split('-')[1])')]")
