  ...
  'New|Changed|Updated' {
    ...
    try {
      ...
      if ($ReleaseNotesTitleNode) {
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesTitleNode.NextSibling; $Node -and $Node.Name -ne 'h2' -and -not $Node.InnerHtml.Contains($Prefix); $Node = $Node.NextSibling) { $Node }
        # ReleaseNotes (en-US)
