...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Object1.Links.Where({ try { $_.href.Contains('LANconfig') -and $_.href.EndsWith('.exe') } catch {} }, 'First')[0].href
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    7z.exe e -aoa -ba -bd -y -o"${InstallerFileExtracted}" $InstallerFile 'LANCOM\LANtools\lanconf.cab' | Out-Host
    $InstallerFile2 = Join-Path $InstallerFileExtracted 'lanconf.cab'
    $InstallerFile2Extracted = New-TempFolder
    7z.exe e -aoa -ba -bd -y -o"${InstallerFile2Extracted}" $InstallerFile2 'lanconf.exe' | Out-Host
    $InstallerFile3 = Join-Path $InstallerFile2Extracted 'lanconf.exe'
    # RealVersion
