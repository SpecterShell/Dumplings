function Read-Installer {
  ...
  $InstallerFileExtracted = Expand-TempArchive -Path $InstallerFile -RelativeFilePath 'ADPermissionsReporter.exe'
  try {
    # Version
    $this.CurrentState.Version = (Get-AdvancedInstallerMsiInfo -Path (Join-Path $InstallerFileExtracted 'ADPermissionsReporter.exe') -Architecture x64).ProductVersion
  } finally {
