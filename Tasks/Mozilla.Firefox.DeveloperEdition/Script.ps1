...
# Version
$this.CurrentState.Version = $Object1.LATEST_FIREFOX_RELEASED_DEVEL_VERSION

# RealVersion
$this.CurrentState.RealVersion = $this.CurrentState.Version -replace 'b.+'
...
foreach ($Arch in $ArchMap.GetEnumerator()) {
  ...
  #   InstallerType   = 'exe'
  #   InstallerUrl    = "${Prefix}$($this.CurrentState.Version)/$($Arch.Value)/en-US/Firefox Setup $($this.CurrentState.Version).exe"
  #   InstallerSha256 = $Object2["$($Arch.Value)/en-US/Firefox Setup $($this.CurrentState.Version).exe"]
  #   ProductCode     = "Firefox Developer Edition $($this.CurrentState.Version) ($($Arch.Key) en-US)"
  # }
  switch ($Locales) {
    ...
    'multi' {
      if ($Arch.Key -ne 'arm64') {
        $this.CurrentState.Installer += [ordered]@{
          Architecture  = $Arch.Key
          InstallerType = 'msix'
          InstallerUrl  = "${Prefix}$($this.CurrentState.Version)/$($Arch.Value)/${_}/Firefox Setup $($this.CurrentState.Version).msix"
        }
      }
      continue
    }
  ...
  'New|Changed|Updated' {
    try {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Key   = 'ReleaseNotesUrl'
        Value = $ReleaseNotesUrl = "https://www.mozilla.org/firefox/$($this.CurrentState.RealVersion)beta/releasenotes/"
      }
    } catch {
      $_ | Out-Host
      $this.Log($_, 'Warning')
