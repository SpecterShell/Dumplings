...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://support.goldensoftware.com/hc/en-us/articles/228069868-Surfer-Version-History' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//h4[contains(., '$($this.CurrentState.Version)')]")
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Global:DumplingsStorage.GoldenSoftwareDownloadPage.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href.Contains('Surfer') } catch {} }, 'First')[0].href
}
