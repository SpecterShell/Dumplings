$Object1 = $Global:DumplingsStorage.NxWitnessApps
...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = "$($Object1.updatesPrefix)/$($Object1.releases.buildNumber)/$($Object1.releases.platforms.Where({ $_.name -eq 'windows' })[0].files.Where({ $_.platform -eq 'windows_x64' -and $_.appType -eq 'server' }, 'First')[0].path)"
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      $Object2 = Invoke-RestMethod -Uri 'https://nxvms.com/api/utils/downloads/history'
