...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://getcoldturkey.com/news/' | ConvertFrom-Html

    $ReleaseNotesNode = $Object2.SelectSingleNode("//div[contains(./dt/text(), '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesNode) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = [regex]::Match($ReleaseNotesNode.SelectSingleNode('./dt').InnerText, '(\d{1,2}\W+[a-zA-Z]+\W+20\d{2})').Groups[1].Value | Get-Date -Format 'yyyy-MM-dd'

      # ReleaseNotes (en-US)
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotes'
        Value  = $ReleaseNotesNode.SelectSingleNode('./dd') | Get-TextContent | Format-Text
      }
    } else {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://getcoldturkey.com/files/Cold_Turkey_Installer.exe'
}

# $Object1 = Invoke-WebRequest -Uri 'https://getcoldturkey.com/version/windows/'
