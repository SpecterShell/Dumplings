...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://convertilla.com/history.html' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//h3[contains(text(), 'Convertilla $($this.CurrentState.Version.Split('.')[0..1] -join '.')')]")
    if ($ReleaseNotesTitleNode) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = [datetime]::ParseExact(
        [regex]::Match($ReleaseNotesTitleNode.InnerText, '([a-zA-Z]+\W+\d+\W+20\d{2})').Groups[1].Value,
        [string[]]@(
          'MMMM d, yyyy',
          'MMMM d, yyyy',
          'MMMM d, yyyy',
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://convertilla.com/download/convertilla.exe'
}
