$Release = Invoke-GitHubApi -Uri 'https://api.github.com/repos/AmintaCCCP/GithubStarsManager/releases/latest'
...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Release.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.Contains('Setup') }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
