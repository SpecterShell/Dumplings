...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['Dehelper'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['Dehelper'] = $OldReleases = [ordered]@{}
}

$Object1 = Invoke-RestMethod -Uri 'https://api.frdic.com/api/v2/appsupport/checkversion' -Headers @{
  EudicUserAgent = "/eusoft_maindb_de_win32/$($this.Status.Contains('New') ? $this.LastState.Version : '13.0.0')//"
}
