$Object1 = Invoke-GitHubApi -Uri "https://api.github.com/repos/win32ss/Supermium/releases/latest"
...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x86'
  InstallerUrl = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.Contains('_32') -and $_.name -match 'setup' }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
$this.CurrentState.Installer += $Installer = [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.Contains('_64') -and $_.name -match 'setup' -and $_.name -match 'win10_11' }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
