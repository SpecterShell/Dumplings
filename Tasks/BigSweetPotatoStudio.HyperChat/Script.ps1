$Object1 = Invoke-GitHubApi -Uri "https://api.github.com/repos/BigSweetPotatoStudio/HyperChat/releases/latest"
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Key   = 'ReleaseNotesUrl'
        Value = $ReleaseNotesUrl = "https://github.com/BigSweetPotatoStudio/HyperChat/blob/doc/ChangeLog.md"
      }

      $Object2 = Invoke-RestMethod -Uri "https://raw.githubusercontent.com/BigSweetPotatoStudio/HyperChat/refs/heads/doc/ChangeLog.md" | Convert-MarkdownToHtml
    ...
    try {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'zh-CN'
        Key    = 'ReleaseNotesUrl'
        Value  = $ReleaseNotesUrlCN = "https://github.com/BigSweetPotatoStudio/HyperChat/blob/doc/ChangeLog.zh.md"
      }

      $Object3 = Invoke-RestMethod -Uri "https://raw.githubusercontent.com/BigSweetPotatoStudio/HyperChat/refs/heads/doc/ChangeLog.zh.md" | Convert-MarkdownToHtml
      ...
      if ($ReleaseNotesTitleNode) {
        ...
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotesUrl'
          Value  = $ReleaseNotesUrlCN + '#' + ($ReleaseNotesTitleNode.InnerText -replace '[^a-zA-Z0-9\-\s]+', '' -replace '\s+', '-').ToLower()
        }
      } else {
