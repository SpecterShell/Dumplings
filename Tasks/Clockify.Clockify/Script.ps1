...
function Get-ReleaseNotes {
  try {
    if ($Global:DumplingsStorage.Contains('Clockify') -and $Global:DumplingsStorage.Clockify.Contains($this.CurrentState.Version)) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = $Global:DumplingsStorage.Clockify[$this.CurrentState.Version].ReleaseTime | Get-Date -AsUTC

      # ReleaseNotes (en-US)
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotes'
        Value  = $Global:DumplingsStorage.Clockify[$this.CurrentState.Version].ReleaseNotes
      }
    } else {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://clockify.me/downloads/clockify-setup.msi'
}
