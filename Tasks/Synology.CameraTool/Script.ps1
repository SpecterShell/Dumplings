$Object1 = Invoke-RestMethod -Uri 'https://utyautoupdate.synology.com/getUpdate/SynologyCameraTool?os=windows&bits=64&include_beta=false'
...
# RealVersion
$this.CurrentState.RealVersion = "$($Object1.version.major).$($Object1.version.minor).$($Object1.version.hotfix)-$($Object1.version.build_number -replace '^0+')"
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.synology.com/api/releaseNote/findChangeLog?identify=SynologyCameraTool&lang=en-us' | Read-ResponseContent | ConvertFrom-Json -AsHashtable
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.synology.com/api/releaseNote/findChangeLog?identify=SynologyCameraTool&lang=zh-cn' | Read-ResponseContent | ConvertFrom-Json -AsHashtable
