...
$Object2 = $Object1.SelectSingleNode('//*[@class="SSIC__content" and contains(., "Revo Track")]//div[@class="SSIC__supported-os-card" and @data-os="os-win"]')
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      if ($Global:DumplingsStorage.Contains('RevoTrack') -and $Global:DumplingsStorage.RevoTrack.Contains($this.CurrentState.Version)) {
        # ReleaseNotes (en-US)
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'en-US'
          Key    = 'ReleaseNotes'
          Value  = $Global:DumplingsStorage.RevoTrack[$this.CurrentState.Version].ReleaseNotes
        }
        # ReleaseNotes (zh-CN)
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotes'
          Value  = $Global:DumplingsStorage.RevoTrack[$this.CurrentState.Version].ReleaseNotesCN
        }
      } else {
