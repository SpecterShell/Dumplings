function Get-ReleaseNotes {
  try {
    if ($Global:DumplingsStorage.Contains('360DrvMgr') -and $Global:DumplingsStorage.'360DrvMgr'.Contains($this.CurrentState.Version)) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = $Global:DumplingsStorage.'360DrvMgr'[$this.CurrentState.Version].ReleaseTime

      # ReleaseNotes (zh-CN)
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'zh-CN'
        Key    = 'ReleaseNotes'
        Value  = $Global:DumplingsStorage.'360DrvMgr'[$this.CurrentState.Version].ReleaseNotesCN
      }
    } else {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://sfdl.360safe.com/drvmgr/guanwang__360DrvMgrInstaller_beta.exe'
}
