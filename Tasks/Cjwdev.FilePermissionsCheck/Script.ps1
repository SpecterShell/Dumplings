function Read-Installer {
  ...
  $InstallerFileExtracted = Expand-TempArchive -Path $InstallerFile -RelativeFilePath 'FilePermissionsCheckInstaller.exe' -CollisionAction Rename
  try {
    # Version
    $this.CurrentState.Version = (Get-AdvancedInstallerMsiInfo -Path (Join-Path $InstallerFileExtracted 'FilePermissionsCheckInstaller.exe') -Architecture x64).DisplayVersion
  } finally {
...
function Get-ReleaseNotes {
  try {
    $Object3 = Invoke-RestMethod -Uri 'https://cjwdev.com/Software/FilePermissions/LatestVersionV2.xml'
...
$Prefix = 'https://cjwdev.com/Software/FilePermissions/Download.html'
