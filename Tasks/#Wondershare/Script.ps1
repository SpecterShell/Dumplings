$Temp.WondershareUpgradeInfo = [ordered]@{}
...
@('x64', 'x86').ForEach(
  ...
  {
    $Temp.WondershareUpgradeInfo[$_.pid.ToString()] = [ordered]@{
      # Version
      Version   = $_.version
      # Installer
      Installer = @()
      ...
      Locale    = @(
        [ordered]@{
          ...
          Key    = 'ReleaseNotes'
          Value  = $_.whats_new_content | ConvertFrom-Html | Get-TextContent | Format-Text
        }
      )
    }
  }
