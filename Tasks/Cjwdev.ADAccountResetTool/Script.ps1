function Read-Installer {
  ...
  $InstallerFileExtracted = Expand-TempArchive -Path $InstallerFile -RelativeFilePath 'AccountResetInstaller.exe' -CollisionAction Rename
  try {
    # Version
    $this.CurrentState.Version = (Get-AdvancedInstallerMsiInfo -Path (Join-Path $InstallerFileExtracted 'AccountResetInstaller.exe') -Architecture x64).DisplayVersion
  } finally {
