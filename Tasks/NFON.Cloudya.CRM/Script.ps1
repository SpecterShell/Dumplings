...
$InstallerUrl = $Object1.Links.Where({ try { $_.href.Contains('cloudya-') -and $_.href.EndsWith('.zip') -and $_.href.Contains('msi') -and $_.href.Contains('win') -and $_.href.Contains('crm') } catch {} }, 'First')[0].href
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "cloudya-$($this.CurrentState.Version)-crm.msi"
    }
  )
}
