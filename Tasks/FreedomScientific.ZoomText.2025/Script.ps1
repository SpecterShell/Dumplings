$Object1 = curl -fsSLA $DumplingsInternetExplorerUserAgent 'https://support.freedomscientific.com/Downloads/OfflineInstallers/GetInstallers?product=ZoomText&version=19&language=enu&releaseType=Offline' | Join-String -Separator "`n" | ConvertFrom-Json
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    $Object2 = curl -fsSLA $DumplingsInternetExplorerUserAgent 'https://support.freedomscientific.com/Downloads/ZoomText/PreviousFeatures?version=2025' | Join-String -Separator "`n" | ConvertFrom-Html
