...
$this.CurrentState.Installer += [ordered]@{
  Architecture         = 'x86'
  InstallerUrl         = $Object1.assets.Where({ $_.name.EndsWith('.zip') -and $_.name.Contains('Windows') -and $_.name.Contains('i386') })[0].browser_download_url | ConvertTo-UnescapedUri
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath     = "twitch-cli_$($this.CurrentState.Version)_Windows_i386\twitch.exe"
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath     = "twitch-cli_$($this.CurrentState.Version)_Windows_x86_64\twitch.exe"
      PortableCommandAlias = 'twitch'
    }
  )
}
