...
$this.CurrentState.Locale += [ordered]@{
  Key   = 'ReleaseNotesUrl'
  Value = $ReleaseNotesUrl = Get-RedirectedUrl1st -Uri 'https://www.tableau.com/support/releases/desktop/latest' -Headers @{ Accept = '*/*'; 'User-Agent' = $DumplingsDefaultUserAgent }
}
# ReleaseNotesUrl (zh-CN)
$this.CurrentState.Locale += [ordered]@{
  Locale = 'zh-CN'
  Key    = 'ReleaseNotesUrl'
  Value  = $ReleaseNotesUrlCN = Get-RedirectedUrl1st -Uri 'https://www.tableau.com/zh-cn/support/releases/desktop/latest' -Headers @{ Accept = '*/*'; 'User-Agent' = $DumplingsDefaultUserAgent }
}
