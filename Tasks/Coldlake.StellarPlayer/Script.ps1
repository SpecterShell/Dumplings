...
switch -Regex ($this.Check()) {
  ...
  'Updated' {
    ...
    if (-not $Global:LocalStorage.Contains("StellarPlayerSubmitting-$($this.CurrentState.Version)")) {
      $Global:LocalStorage["StellarPlayerSubmitting-$($this.CurrentState.Version)"] = $ToSubmit = $true
    }
    $Mutex.ReleaseMutex()
    $Mutex.Dispose()
