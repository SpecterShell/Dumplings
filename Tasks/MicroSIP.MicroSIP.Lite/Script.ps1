...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.Contains('.exe') -and $_.href.Contains('Lite') } catch {} }, 'First')[0].href | Split-Uri -LeftPart 'Path'
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      # ReleaseTime
      $this.CurrentState.ReleaseTime = [regex]::Match($Object2.SelectSingleNode("//tr[contains(./th, 'Date')]/td[2]").InnerText, '([a-zA-Z]+\W+\d{1,2}\W+20\d{2})').Groups[1].Value | Get-Date -Format 'yyyy-MM-dd'
