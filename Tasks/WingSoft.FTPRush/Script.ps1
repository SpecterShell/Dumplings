...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://www.wftpserver.com/releasenotes.htm' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//*[@class='container']//h5[contains(., '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesTitleNode) {
      ...
      # ReleaseNotes (en-US)
      $ReleaseNotesNodes = for ($Node = $ReleaseNotesTitleNode.NextSibling; $Node -and $Node.Name -ne 'h5'; $Node = $Node.NextSibling) { $Node }
...
$Prefix = 'https://www.wftpserver.com/download.htm'
$Object1 = Invoke-WebRequest -Uri $Prefix

$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href -match 'FTPRush' } catch {} }, 'First')[0].href
}
