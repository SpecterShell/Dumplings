...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['RevoMeasure'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['RevoMeasure'] = $OldReleases = [ordered]@{}
}
...
$Object1 = Invoke-RestMethod -Uri 'https://api.infly3d.com/software/version/info' -Method Post -Body (
  @{
    deploy                = 'test'
    sn_code               = 'FFFFFFFFFFFFFS051'
    user_version_string   = $this.Status.Contains('New') ? 'v1.0.0.0325' : "v$($this.LastState.Version)"
