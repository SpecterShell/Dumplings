$Object1 = Invoke-GitHubApi -Uri 'https://api.github.com/repos/hadc188/reader/releases/latest'
    ...
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node; $Node = $Node.NextSibling) {
          if ($Node.Name -in @('h2', 'h3') -and $Node.InnerText -match '下载说明') {
            $Node = $Node.SelectSingleNode("./following-sibling::$($Node.Name)/preceding-sibling::*[1]")
          } else {
            $Node
          }
        }
