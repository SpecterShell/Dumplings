...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      if ($Global:DumplingsStorage.Contains('ZWCAD2026') -and $Global:DumplingsStorage['ZWCAD2026'].Contains($this.CurrentState.RealVersion)) {
        # ReleaseNotes (en-US)
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'en-US'
          Key    = 'ReleaseNotes'
          Value  = $Global:DumplingsStorage['ZWCAD2026'][$this.CurrentState.RealVersion].ReleaseNotesEN
        }
        # ReleaseNotes (zh-CN)
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotes'
          Value  = $Global:DumplingsStorage['ZWCAD2026'][$this.CurrentState.RealVersion].ReleaseNotesCN
        }
      } else {
