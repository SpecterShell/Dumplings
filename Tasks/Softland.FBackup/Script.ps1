...
function Get-ReleaseNotes {
  try {
    if ($Global:DumplingsStorage.Contains('FBackup') -and $Global:DumplingsStorage.FBackup.Contains($this.CurrentState.Version)) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = $Global:DumplingsStorage.FBackup[$this.CurrentState.Version].ReleaseTime | Get-Date -AsUTC

      # ReleaseNotes (en-US)
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotes'
        Value  = $Global:DumplingsStorage.FBackup[$this.CurrentState.Version].ReleaseNotes
      }
    } else {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://download.fbackup.com/download/setup/fbsetup-full.exe'
}
