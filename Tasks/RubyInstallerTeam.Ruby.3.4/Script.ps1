...
$this.CurrentState.Installer += [ordered]@{
  Architecture           = 'arm64'
  InstallerUrl           = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and -not $_.name.Contains('devkit') -and $_.name.Contains('arm') }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
  AppsAndFeaturesEntries = @(
    [ordered]@{
