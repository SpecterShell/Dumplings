...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.StartsWith('Miniforge3') -and $_.name.Contains('x86_64') -and -not $_.name.Contains('pypy') }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    $this.CurrentState.Installer[0]['ProductCode'] = "Miniforge3 $($this.CurrentState.Version) (Python ${PythonVersion} 64-bit)"
