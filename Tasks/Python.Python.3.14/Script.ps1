# x86
$Object1 = $Global:DumplingsStorage.PythonVersions.versions.Where({ $_.'install-for'.Contains('3.14-32') -and $_.'sort-version' -match '^\d+(?:\.\d+)+$' }, 'First')[0]
# x64
$Object2 = $Global:DumplingsStorage.PythonVersions.versions.Where({ $_.'install-for'.Contains('3.14-64') -and $_.'sort-version' -match '^\d+(?:\.\d+)+$' }, 'First')[0]
# arm64
$Object3 = $Global:DumplingsStorage.PythonVersions.versions.Where({ $_.'install-for'.Contains('3.14-arm64') -and $_.'sort-version' -match '^\d+(?:\.\d+)+$' }, 'First')[0]
