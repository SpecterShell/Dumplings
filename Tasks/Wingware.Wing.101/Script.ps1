$Object1 = Invoke-RestMethod -Uri 'https://wingware.com/update' -Method Post -Body @{
  ...
  prod         = 'wing-101'
  osname       = 'win32'
  from_version = $this.LastState.Contains('Version') ? $this.LastState.Version : '10.0.5.0'
  page         = 'update'
  noheader     = '1'
}
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = "https://wingware.com/pub/wing-101/$($this.CurrentState.Version)/wing-101-$($this.CurrentState.Version).exe"
  ProductCode  = "Wing 101 $($this.CurrentState.Version.Split('.')[0])_is1"
}
  ...
  'New|Changed|Updated' {
    try {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Key   = 'ReleaseNotesUrl'
        Value = $ReleaseNotesUrl = "https://wingware.com/pub/wing-101/$($this.CurrentState.Version)/CHANGELOG.txt"
      }
