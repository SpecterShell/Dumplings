...
if (Test-Path -Path $OldReleaseNotesPath) {
  $Global:DumplingsStorage['Frhelper'] = $OldReleaseNotes = Get-Content -Path $OldReleaseNotesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['Frhelper'] = $OldReleaseNotes = [ordered]@{}
}

$Object1 = Invoke-RestMethod -Uri 'https://api.frdic.com/api/v2/appsupport/checkversion' -Headers @{
  EudicUserAgent = "/eusoft_maindb_fr_win32/$($this.LastState.Contains('Version') ? $this.LastState.Version : '13.0.0')//"
}
