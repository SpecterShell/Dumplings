$Object1 = Invoke-GitHubApi -Uri 'https://api.github.com/repos/Technicalflight/AiGuard/releases/latest'
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node -and $Node.InnerText -notmatch '安装包|平台差异|隐私红线'; $Node = $Node.NextSibling) { $Node }
        # ReleaseNotes (zh-CN)
