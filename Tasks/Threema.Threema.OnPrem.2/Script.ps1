...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = "https://releases.threema.ch/desktop/$($this.CurrentState.Version)/threema-onprem-desktop-v$($this.CurrentState.Version)-windows-x64.msix"
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      ...
      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//div[contains(@id, 'accordion') and contains(@id, 'heading') and contains(., 'Threema OnPrem 2.0 for Desktop') and contains(., 'Beta $([regex]::Match($this.CurrentState.Version, 'beta(\d+)').Groups[1].Value)')]")
