$Object1 = Invoke-GitHubApi -Uri "https://api.github.com/repos/Anxcye/anx-reader/releases/latest"
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $Node = $null
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.SelectSingleNode('/ul/li[1]'); $Node -and $Node.InnerText -notmatch "[${CJK}]"; $Node = $Node.NextSibling) { $Node }
        # ReleaseNotes (en-US)
