...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['RevoTrack'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['RevoTrack'] = $OldReleases = [ordered]@{}
}
...
$Object1 = Invoke-RestMethod -Uri 'https://api.infly3d.com/software/version/info' -Method Post -Body (
  @{
    deploy                = 'test'
    sn_code               = 'FFFFFFFFFFFFFT051'
    user_version_string   = $this.Status.Contains('New') ? 'v5.7.4.658' : "v$($this.LastState.Version)"
