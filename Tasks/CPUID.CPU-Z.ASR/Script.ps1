# Version
$this.CurrentState.Version = [regex]::Match($Global:DumplingsStorage.CPUZDownloadPage.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href.Contains('-asr') } catch {} }, 'First')[0].href, '(\d+(?:\.\d+)+)').Groups[1].Value

# Installer
$this.CurrentState.Installer += [ordered]@{
  InstallerLocale = 'en-US'
  InstallerUrl    = "https://download.cpuid.com/cpu-z/cpu-z_$($this.CurrentState.Version)-asr-en.exe"
}
