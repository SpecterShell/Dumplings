...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//h4[contains(text(), 'Javelin') and (contains(text(), '$($this.CurrentState.Version)') or contains(text(), '$('{0}.{1}.{2}{3}' -f $this.CurrentState.Version.Split('.'))'))]")
