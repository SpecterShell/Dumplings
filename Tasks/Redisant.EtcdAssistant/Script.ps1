...
$RepoName = 'etcd-assistant-release'
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "$($Asset.name | Split-Path -LeafBase)\EA-$($this.CurrentState.Version).exe"
    }
  )
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.redisant.com/etcd/download' | ConvertFrom-Html
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.redisant.cn/etcd/download' | ConvertFrom-Html
