...
# Version
$this.CurrentState.Version = $Version = $Object1.FIREFOX_ESR.Replace('esr', '')
...
foreach ($Arch in $ArchMap.GetEnumerator()) {
  ...
  #   InstallerType   = 'exe'
  #   InstallerUrl    = "${Prefix}${Version}esr/$($Arch.Value)/en-US/Firefox Setup ${Version}esr.exe"
  #   InstallerSha256 = $Object2["$($Arch.Value)/en-US/Firefox Setup ${Version}esr.exe"]
  #   ProductCode     = "Mozilla Firefox ${Version} ESR ($($Arch.Key) en-US)"
  # }
  switch ($Locales) {
    ...
    'multi' {
      $this.CurrentState.Installer += [ordered]@{
        Architecture  = $Arch.Key
        InstallerType = 'msix'
        InstallerUrl  = "${Prefix}${Version}esr/$($Arch.Value)/${_}/Firefox Setup ${Version}esr.msix"
      }
      continue
    }
    ...
    'ast' {
      $this.CurrentState.Installer += [ordered]@{
        ...
        Architecture    = $Arch.Key
        InstallerType   = 'exe'
        InstallerUrl    = "${Prefix}${Version}esr/$($Arch.Value)/${_}/Firefox Setup ${Version}esr.exe"
        InstallerSha256 = $Object2["$($Arch.Value)/${_}/Firefox Setup ${Version}esr.exe"]
        ProductCode     = "Mozilla Firefox ${Version} ESR ($($Arch.Key) ${_})"
      }
      continue
    }
