...
$this.CurrentState.Installer += [ordered]@{
  Architecture           = 'x86'
  InstallerUrl           = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name -match '[^a-zA-Z0-9]gcc[^a-zA-Z0-9]' })[0].browser_download_url | ConvertTo-UnescapedUri
...
$this.CurrentState.Installer += [ordered]@{
  Architecture           = 'x64'
  InstallerUrl           = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.Contains('gcc64') })[0].browser_download_url | ConvertTo-UnescapedUri
