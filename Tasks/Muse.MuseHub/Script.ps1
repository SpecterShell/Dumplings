...
'@
    ...
    $Object2 = Invoke-RestMethod -Uri 'https://cosmos-customer-webservice.azurewebsites.net/graphql' -Method Post -Body (
      @{ query = $Query } | ConvertTo-Json -Compress
    ) -ContentType 'application/json'

    if ($Object2.data.product.whatIsNew.Contains($this.CurrentState.Version.Split('.')[0..2] -join '.')) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = $Object2.data.product.releaseDate.ToUniversalTime()

      # ReleaseNotes (en-US)
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotes'
        Value  = $Object2.data.product.whatIsNew | Convert-MarkdownToHtml -Extensions 'advanced', 'emojis', 'hardlinebreak' | Get-TextContent | Format-Text
      }
    } else {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://muse-cdn.com/Muse_Hub.exe'
}
