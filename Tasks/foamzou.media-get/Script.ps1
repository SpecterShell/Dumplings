...
if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
  ...
  for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node -and -not $Node.InnerText.Contains('New Contributors') -and -not $Node.InnerText.Contains('Full Changelog:'); $Node = $Node.NextSibling) {
