...
$this.CurrentState.Installer += [ordered]@{
  ...
  InstallerUrl    = $InstallerUrlCN = $Object2.Links.Where({ try { $_.href -match 'seafile-\d+(?:\.\d+)+\.msi$' } catch {} }, 'First')[0].href
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/haiwen/seafile-admin-docs/HEAD/manual/changelog/client-changelog.md' | Convert-MarkdownToHtml
