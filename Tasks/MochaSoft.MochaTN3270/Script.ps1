...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://www.mochasoft.dk/tn3270_release.htm' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//b[contains(text(), 'version $($this.CurrentState.Version -replace '(?<=^\d+\.\d+)(\.0)+')')]")
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://www.mochasoft.dk/tn3270.msi'
}
