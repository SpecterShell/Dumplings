...
$FolderName = $Object1.Links.Where({ try { $_.href -match '^/terraform/\d+(?:\.\d+)+/$' } catch {} }).href | Sort-Object -Property { [ChunkVersion]($_) } -Bottom 1
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      $Object2 = Invoke-GitHubApi -Uri "https://api.github.com/repos/hashicorp/terraform/releases/tags/v$($this.CurrentState.Version)"
