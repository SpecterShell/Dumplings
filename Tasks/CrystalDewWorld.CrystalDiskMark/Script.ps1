...
$PatternFilename = 'CrystalDiskMark\d+(?:_\d+)+[a-zA-Z]?.*\.exe'
...
if ($Assets2 = $Object1.Where({ $_.title.'#cdata-section' -eq "${RootPath}/$($this.CurrentState.Version)/CrystalDiskMark$($this.CurrentState.Version.Replace('.', '_')).exe" })) {
  $Assets = $Assets2
}
