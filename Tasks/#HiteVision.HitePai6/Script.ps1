...
if (Test-Path -Path $OldReleaseNotesPath) {
  $Global:LocalStorage['HitePai6'] = $OldReleaseNotes = Get-Content -Path $OldReleaseNotesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:LocalStorage['HitePai6'] = $OldReleaseNotes = [ordered]@{}
}
