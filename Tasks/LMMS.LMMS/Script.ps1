$Object1 = Invoke-GitHubApi -Uri "https://api.github.com/repos/LMMS/lmms/releases/latest"
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node; ) {
          if ($Node.InnerText.Contains('Previous features and fixes')) {
            $Node = $Node.SelectSingleNode('./following-sibling::p[./strong]')
          } else {
            $Node
