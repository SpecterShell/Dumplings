...
function Get-ReleaseNotes {
  try {
    ...
    $this.CurrentState.Locale += [ordered]@{
      Locale = 'en-US'
      Key    = 'ReleaseNotesUrl'
      Value  = $ReleaseNotesUrl = 'https://cavalry.studio/docs/'
    }

    $Object3 = Invoke-WebRequest -Uri $ReleaseNotesUrl
    $ReleaseNotesUrl = Join-Uri $ReleaseNotesUrl $Object3.Links.Where({ try { $_.href.Contains($this.CurrentState.Version.Replace('.', '-')) } catch {} }, 'First')[0].href
...
$Prefix = 'https://cavalry.studio/en/'
$Object1 = Invoke-WebRequest -Uri $Prefix

$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.EndsWith('.msi') } catch {} }, 'First')[0].href | ConvertTo-Https
}
