$Object1 = (Invoke-WebRequest -Uri 'https://julialang-s3.julialang.org/bin/versions.json' | Read-ResponseContent | ConvertFrom-Json -AsHashtable).GetEnumerator().Where({ $_.Value.stable }) | Sort-Object -Property { [ChunkVersion]($_.Key) } -Bottom 1
