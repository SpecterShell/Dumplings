...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['HiSuiteCN'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['HiSuiteCN'] = $OldReleases = [ordered]@{}
}
...
<root>
  <rule name="DashBoard">$($this.Status.Contains('New') ? '11.0.0.000' : $this.LastState.Version)</rule>
