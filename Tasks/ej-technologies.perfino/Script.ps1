$Prefix = 'https://download.ej-technologies.com/perfino/'
...
$Object1 = (Invoke-RestMethod -Uri "${Prefix}updates${MajorVersion}.xml").updateDescriptor.entry.Where({ $_.targetMediaFileId -eq '23' }, 'First')[0]
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object2 = (Invoke-RestMethod -Uri 'https://www.ej-technologies.com/feeds/perfino').Where({ $_.title.Contains($this.CurrentState.Version) }, 'First')
