...
switch ($this.Check()) {
  ({ $_ -ge 1 }) {
    try {
      ...
      $ReleaseNotesNode = $Object2.Where({ $_.html.html.Contains("V$($this.CurrentState.Version.Split('.')[0..1] -join '.')") })
      if ($ReleaseNotesNode) {
    ...
    try {
      ...
      $ReleaseNotesCNNode = $Object3.Where({ $_.html.html.Contains("V$($this.CurrentState.Version.Split('.')[0..1] -join '.')") })
