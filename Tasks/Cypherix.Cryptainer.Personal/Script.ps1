$Prefix = 'https://www.cypherix.com/cryptainer-pe-download-center.html'
...
# Version
$this.CurrentState.Version = [regex]::Match($this.CurrentState.Installer[0].InstallerUrl, 'cryptainerpe(\d+(?:\.\d+)+)').Groups[1].Value
...
switch -Regex ($this.Check()) {
  ...
  'Updated' {
    if ($this.CurrentState.Version.Split('.')[0] -ne $this.Config.WinGetIdentifier.Split('.')[-1]) {
      ...
      $this.CurrentState.Installer[0].ProductCode = "cryptainerpe$($this.CurrentState.Version.Split('.')[0])_is1"
    }
    $this.Submit()
  }
}
