...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://ttyplus.com/multi-tabbed-putty/changelog/' | ConvertFrom-Html
    $Object3 = [System.IO.StringReader]::new($Object2.SelectSingleNode('//textarea').InnerText)

    while ($Object3.Peek() -ne -1) {
      $String = $Object3.ReadLine()
      if ($String -match "^Ver $([regex]::Escape($this.CurrentState.Version))") {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://ttyplus.com/download/mtputty_setup.exe'
}
