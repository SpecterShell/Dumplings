...
$RepoName = 'pulsar-assistant-release'
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "PA-$($this.CurrentState.Version)-windows\PA-$($this.CurrentState.Version).exe"
    }
  )
}
...
switch ($this.Check()) {
  ({ $_ -ge 1 }) {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.redisant.com/pa/download' | ConvertFrom-Html
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.redisant.cn/pa/download' | ConvertFrom-Html
