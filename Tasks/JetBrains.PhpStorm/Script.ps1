$Object1 = $Global:DumplingsStorage.JetBrainsApps.PS.release
  ...
  'New|Changed|Updated' {
    try {
      ...
      if ($Object1.notesLink) {
        ...
        $this.CurrentState.Locale += [ordered]@{
          Key   = 'ReleaseNotesUrl'
          Value = 'https://www.jetbrains.com/phpstorm/whatsnew/'
        }
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotesUrl'
          Value  = 'https://www.jetbrains.com/zh-cn/phpstorm/whatsnew/'
        }
      }
    } catch {
      $_ | Out-Host
