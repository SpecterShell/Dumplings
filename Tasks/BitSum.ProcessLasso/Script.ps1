...
function Get-ReleaseNotes {
  try {
    $Object4 = Invoke-WebRequest -Uri 'https://bitsum.com/changes/processlasso/' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object4.SelectSingleNode("//p[contains(./strong/text(), '$($this.CurrentState.Version)')]")
...
$this.CurrentState.Installer += $InstallerX86 = [ordered]@{
  Architecture = 'x86'
  InstallerUrl = $Object1.Links.Where({ try { $_.href.EndsWith('.exe') -and -not $_.href.Contains('beta') -and $_.href.Contains('32') } catch {} }, 'First')[0].href
}
...
$this.CurrentState.Installer += $InstallerX64 = [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Object1.Links.Where({ try { $_.href.EndsWith('.exe') -and -not $_.href.Contains('beta') -and $_.href.Contains('64') } catch {} }, 'First')[0].href
}
