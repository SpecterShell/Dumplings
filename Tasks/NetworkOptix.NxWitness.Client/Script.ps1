...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = "$($Object1.updatesPrefix)/$($Object1.releases.buildNumber)/$($Object1.releases.platforms.Where({ $_.name -eq 'windows' })[0].files.Where({ $_.platform -eq 'windows_x64' -and $_.appType -eq 'client' }, 'First')[0].path)"
}
