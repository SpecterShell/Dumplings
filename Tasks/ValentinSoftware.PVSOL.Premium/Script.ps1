...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Get-RedirectedUrl -Uri 'https://valentin-software.com/downloads/pvsol-premium/latest'
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://valentin-software.com/en/products/pvsol-premium/' | ConvertFrom-Html

      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//div[@class='title' and contains(text(), 'PV*SOL premium $($this.CurrentState.Version.Split('.')[0]) Release $($this.CurrentState.Version.Split('.')[1])')]")
  ...
  'Updated' {
    if ($this.CurrentState.Version.Split('.')[0] -ne $this.Config.WinGetIdentifier.Split('.')[-1]) {
      ...
      $this.CurrentState.Installer[0].ProductCode = "PVSOL premium $($this.CurrentState.Version.Split('.')[0])_is1"
    }
    $this.Submit()
  }
}
