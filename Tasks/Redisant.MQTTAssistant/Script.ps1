...
$RepoName = 'mqtt-assistant-release'
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "MA-$($this.CurrentState.Version)-windows\MA-$($this.CurrentState.Version).exe"
    }
  )
}
...
switch ($this.Check()) {
  ({ $_ -ge 1 }) {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.redisant.com/mqtt/download' | ConvertFrom-Html
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.redisant.cn/mqtt/download' | ConvertFrom-Html
