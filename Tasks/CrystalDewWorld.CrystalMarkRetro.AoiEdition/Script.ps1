...
$PatternFilename = 'CrystalMarkRetro\d+(?:_\d+)+[a-zA-Z]?Aoi.*\.exe'
...
if ($Assets2 = $Object1.Where({ $_.title.'#cdata-section' -eq "${RootPath}/$($this.CurrentState.Version)/CrystalMarkRetro$($this.CurrentState.Version.Replace('.', '_'))Aoi.exe" })) {
  $Assets = $Assets2
}
