$Object1 = Invoke-WebRequest -Uri 'https://im.dingtalk.com/manifest/new/release_windows_vista_later_manual_lowpriority.json' | Read-ResponseContent | ConvertFrom-Json
...
switch -Regex ($this.Check()) {
  ...
  'Updated' {
    ...
    if (-not $Global:DumplingsStorage.Contains("DingTalkSubmitting-$($this.CurrentState.Version)")) {
      $Global:DumplingsStorage["DingTalkSubmitting-$($this.CurrentState.Version)"] = $ToSubmit = $true
    }
    $Mutex.ReleaseMutex()
    $Mutex.Dispose()
