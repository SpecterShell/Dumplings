...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "RA-$($this.CurrentState.Version)-windows\RA-$($this.CurrentState.Version).exe"
    }
  )
}
...
switch ($this.Check()) {
  ({ $_ -ge 1 }) {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.redisant.com/download' | ConvertFrom-Html
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.redisant.cn/download' | ConvertFrom-Html
