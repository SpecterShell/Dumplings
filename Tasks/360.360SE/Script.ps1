...
switch (Compare-State) {
  ({ $_ -ge 1 }) {
    $Object2 = Invoke-WebRequest -Uri 'https://bbs.360.cn/thread-15948589-1-1.html' | ConvertFrom-Html
    ...
    try {
      ...
      if ($ReleaseNotesTitle.Contains($Task.CurrentState.Version)) {
        ...
        $Task.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotes'
          Value  = $Object2.SelectNodes('//*[@id="postmessage_118145577"]/strong[3]/following-sibling::node()[count(.|//*[@id="postmessage_118145577"]/strong[4]/preceding-sibling::node())=count(//*[@id="postmessage_118145577"]/strong[4]/preceding-sibling::node())]') | Get-TextContent | Format-Text
        }
      } else {
