$Object1 = Invoke-GitHubApi -Uri "https://api.github.com/repos/stardustai/dataset-viewer/releases/latest"
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node -and -not ($Node.Name -match '^h\d$' -and $Node.InnerText.Contains('Download')); $Node = $Node.NextSibling) { $Node }
        # ReleaseNotes (en-US)
