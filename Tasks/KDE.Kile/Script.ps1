...
$RootPath = '/unstable'
$PatternPath = '.+'
$PatternFilename = 'kile-((\d+(?:\.\d+)+)-\d+).+\.exe'
...
# RealVersion
$this.CurrentState.RealVersion = [regex]::Match($Assets[0].title.'#cdata-section', "^$([regex]::Escape($RootPath))/${PatternPath}/${PatternFilename}").Groups[2].Value
