function Read-Installer {
  ...
  $InstallerFileExtracted = Expand-TempArchive -Path $InstallerFile -RelativeFilePath 'ProcessPrompt.exe'
  try {
    # Version
    $this.CurrentState.Version = (Get-AdvancedInstallerMsiInfo -Path (Join-Path $InstallerFileExtracted 'ProcessPrompt.exe') -Architecture x64).ProductVersion
  } finally {
