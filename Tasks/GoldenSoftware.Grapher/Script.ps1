...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://support.goldensoftware.com/hc/en-us/articles/228070288-Grapher-Version-History' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//p[contains(./strong, '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesTitleNode) {
      ...
      # ReleaseNotes (en-US)
      $ReleaseNotesNodes = for ($Node = $ReleaseNotesTitleNode.NextSibling; $Node -and -not $Node.SelectSingleNode('./self::p/strong'); $Node = $Node.NextSibling) { $Node }
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Global:DumplingsStorage.GoldenSoftwareDownloadPage.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href.Contains('Grapher') } catch {} }, 'First')[0].href
}
