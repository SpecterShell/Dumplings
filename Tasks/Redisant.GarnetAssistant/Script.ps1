...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "$($Asset.name | Split-Path -LeafBase)\GA-$($this.CurrentState.Version).exe"
    }
  )
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.redisant.com/garnet/download' | ConvertFrom-Html
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.redisant.cn/garnet/download' | ConvertFrom-Html
