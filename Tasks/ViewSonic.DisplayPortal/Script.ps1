function Read-Installer {
  ...
  7z.exe x -aoa -ba -bd -y -o"${InstallerFileExtracted}" $InstallerFile 'Display_Portal_win\setup.exe' | Out-Host
  $InstallerFile2 = Join-Path $InstallerFileExtracted 'Display_Portal_win\setup.exe' -Resolve
  # Version
...
$Prefix = 'https://www.viewsonic.com/global/products/lcd/ViewSonic%20Display%20Portal'
$Object1 = Invoke-WebRequest -Uri $Prefix

$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.EndsWith('.zip') -and $_.href -match '_win' } catch {} }, 'First')[0].href
}
