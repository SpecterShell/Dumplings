...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Global:DumplingsStorage.Axes4DownloadPage.Links.Where({ try { $_.href.Contains('axesPDF') -and $_.href -match 'Setup' -and $_.OuterHTML -notmatch 'beta' } catch {} }, 'First')[0].href
}
  ...
  'New|Changed|Updated' {
    try {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotesUrl'
        Value  = 'https://support.axes4.com/hc/sections/7371445994770'
      }

      $ReleaseNotesUrl = 'https://support.axes4.com/hc/en-us/sections/7371445994770-Release-Notes'
