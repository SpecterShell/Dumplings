$Object1 = [System.Net.Http.Headers.ContentDispositionHeaderValue](Invoke-WebRequest -Uri 'https://islonline.net/users/download/ISLPronto?platform=win32' -Method Head).Headers.'Content-Disposition'[0]
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = "https://islonline.net/users/download/ISLPronto?platform=win32&version=$($this.CurrentState.Version)"
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//h4[contains(., 'ISL Pronto $($this.CurrentState.Version)')]")
