function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://enterprisedt.com/products/completeftp/doc/guide/html/history.html' | ConvertFrom-Html

    $ReleaseNotesNode = $Object2.SelectSingleNode("//tr[contains(./td[1], '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesNode) {
      # ReleaseTime
...
$Prefix = 'https://enterprisedt.com/products/completeftp/download/windows/'
$Object1 = Invoke-WebRequest -Uri $Prefix

# Installer
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ $_.href.EndsWith('.exe') }, 'First')[0].href
}
