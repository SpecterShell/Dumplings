...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['RevoMirror'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['RevoMirror'] = $OldReleases = [ordered]@{}
}
...
$Object1 = Invoke-RestMethod -Uri 'https://api.infly3d.com/software/version/info' -Method Post -Body (
  @{
    deploy                = 'test'
    sn_code               = 'FFFFFFFFFFFFFR011'
    user_version_string   = $this.Status.Contains('New') ? 'v1.0.0.25' : "v$($this.LastState.Version)"
