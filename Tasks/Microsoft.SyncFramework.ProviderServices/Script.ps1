...
# x86
$Object2 = $Object1.dlcDetailsView.downloadFile.Where({ $_.name.EndsWith('.msi') -and $_.name -match 'ProviderServices' -and $_.name -match 'x86' }, 'First')[0]
# x64
$Object3 = $Object1.dlcDetailsView.downloadFile.Where({ $_.name.EndsWith('.msi') -and $_.name -match 'ProviderServices' -and $_.name -match 'x64' }, 'First')[0]
  ...
  'Changed|Updated' {
    foreach ($Locale in @('de-DE', 'es-ES', 'fr-FR', 'it-IT', 'ja-JP', 'ko-KR', 'pt-BR', 'ru-RU', 'zh-CN', 'zh-TW')) {
      ...
      $this.CurrentState.Installer += [ordered]@{
        ...
        Architecture    = 'x86'
        InstallerUrl    = $Object4.dlcDetailsView.downloadFile.Where({ $_.name.EndsWith('.msi') -and $_.name -match 'ProviderServices' -and $_.name -match 'x86' }, 'First')[0].url
      }
      $this.CurrentState.Installer += [ordered]@{
        InstallerLocale = $Locale
        Architecture    = 'x64'
        InstallerUrl    = $Object4.dlcDetailsView.downloadFile.Where({ $_.name.EndsWith('.msi') -and $_.name -match 'ProviderServices' -and $_.name -match 'x64' }, 'First')[0].url
      }
    }
