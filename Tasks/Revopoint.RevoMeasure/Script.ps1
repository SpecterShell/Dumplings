...
$Object2 = $Object1.SelectSingleNode('//*[@class="SSIC__content" and contains(., "Revo Measure")]//div[@class="SSIC__supported-os-card" and @data-os="os-win"]')
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      if ($Global:DumplingsStorage.Contains('RevoMeasure') -and $Global:DumplingsStorage.RevoMeasure.Contains($this.CurrentState.Version)) {
        # ReleaseNotes (en-US)
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'en-US'
          Key    = 'ReleaseNotes'
          Value  = $Global:DumplingsStorage.RevoMeasure[$this.CurrentState.Version].ReleaseNotes
        }
        # ReleaseNotes (zh-CN)
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotes'
          Value  = $Global:DumplingsStorage.RevoMeasure[$this.CurrentState.Version].ReleaseNotesCN
        }
      } else {
