...
$RepoName = 'zookeeper-assistant-release'
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "ZA-$($this.CurrentState.Version)-windows\ZA-$($this.CurrentState.Version).exe"
    }
  )
}
...
switch ($this.Check()) {
  ({ $_ -ge 1 }) {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.redisant.com/za/download' | ConvertFrom-Html
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.redisant.cn/za/download' | ConvertFrom-Html
