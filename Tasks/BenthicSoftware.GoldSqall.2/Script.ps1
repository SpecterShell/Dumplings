...
$this.CurrentState.Installer += [ordered]@{
  ...
  ProductCode  = "GoldSqall$($this.CurrentState.Version.Split('.')[0])32_is1"
}
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  ...
  ProductCode  = "GoldSqall$($this.CurrentState.Version.Split('.')[0])64_is1"
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      $Object3 = Invoke-WebRequest -Uri 'https://www.benthicsoftware.com/goldsqall.html' | ConvertFrom-Html

      $ReleaseNotesTitleNode = $Object3.SelectSingleNode("//*[@id='goldsqall_history_tabpanel']//h2[contains(text(), 'Version $($this.CurrentState.RealVersion.Split('.')[0..1] -join '.') Build $($this.CurrentState.RealVersion.Split('.')[3])')]")
