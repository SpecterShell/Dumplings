...
$this.CurrentState.Installer += [ordered]@{
  Architecture  = 'x86'
  InstallerType = 'wix'
  InstallerUrl  = Get-RedirectedUrl -Uri ($Prefix + $Object3.Where({ $_.EndsWith('.msi') -and $_.Contains('x86') }, 'First')[0])
}
$this.CurrentState.Installer += [ordered]@{
  Architecture  = 'x64'
  InstallerType = 'wix'
  InstallerUrl  = Get-RedirectedUrl -Uri ($Prefix + $Object3.Where({ $_.EndsWith('.msi') -and $_.Contains('x64') }, 'First')[0])
}
$this.CurrentState.Installer += [ordered]@{
  Architecture  = 'x86'
  InstallerType = 'nullsoft'
  InstallerUrl  = Get-RedirectedUrl -Uri ($Prefix + $Object3.Where({ $_.EndsWith('.exe') -and $_.Contains('x86') }, 'First')[0])
}
$this.CurrentState.Installer += [ordered]@{
  Architecture  = 'x64'
  InstallerType = 'nullsoft'
  InstallerUrl  = Get-RedirectedUrl -Uri ($Prefix + $Object3.Where({ $_.EndsWith('.exe') -and $_.Contains('x64') }, 'First')[0])
}
