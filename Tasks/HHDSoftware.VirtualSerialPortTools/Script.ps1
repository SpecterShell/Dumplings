...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://hhdsoftware.com/virtual-serial-port-tools/history' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//div[@class='p-vh-item' and contains(.//div[@class='p-vh-item-header-title'], '$($this.CurrentState.Version)')]")
...
$this.CurrentState.Installer += $Installer = [ordered]@{
  InstallerUrl = 'https://hhdsoftware.com/download/virtual-serial-port-tools.exe'
}
...
$this.CurrentState.Installer += $InstallerArm64 = [ordered]@{
  Architecture = 'arm64'
  InstallerUrl = 'https://hhdsoftware.com/download/virtual-serial-port-tools-arm64.exe'
}
...
if ([datetime]$this.CurrentState.LastModifiedArm64 -eq [datetime]$this.LastState.LastModifiedArm64) {
