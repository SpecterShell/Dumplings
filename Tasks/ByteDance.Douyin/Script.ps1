...
switch -Regex ($this.Check()) {
  ...
  'Updated' {
    ...
    if (-not $Global:LocalStorage.Contains("DouyinSubmitting-$($this.CurrentState.Version)")) {
      $Global:LocalStorage["DouyinSubmitting-$($this.CurrentState.Version)"] = $ToSubmit = $true
    }
    $Mutex.ReleaseMutex()
    $Mutex.Dispose()
