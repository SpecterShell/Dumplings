function Read-Installer {
  ...
  7z.exe e -aoa -ba -bd -y -o"${InstallerFileExtracted}" $InstallerFile 'ImpulseSetup.msi' | Out-Host
  $InstallerFile2 = Join-Path $InstallerFileExtracted 'ImpulseSetup.msi' -Resolve
  # Version
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Global:DumplingsStorage.VictronEnergyPrefix $Global:DumplingsStorage.VictronEnergyDownloadPage.Links.Where({ try { $_.href.EndsWith('.zip') -and $_.href.Contains('Impulse Charger Tool') } catch {} }, 'First')[0].href
}
