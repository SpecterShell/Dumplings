...
$InstallerObjects = $Object1.RDF.Content |
  Where-Object -FilterScript { $_.about.EndsWith('.exe') -and $_.about.Contains('Q16') -and -not $_.about.Contains('HDRI') -and $_.about.Contains('dll') } |
...
if ($VersionX86 -eq $LatestVersion) {
  ...
  $this.CurrentState.Installer += [ordered]@{
    ...
    ProductCode     = "ImageMagick $($LatestVersion.Split('-')[0]) Q16 (32-bit)_is1"
  }
}
if ($VersionX64 -eq $LatestVersion) {
  ...
  $this.CurrentState.Installer += [ordered]@{
    ...
    ProductCode     = "ImageMagick $($LatestVersion.Split('-')[0]) Q16 (64-bit)_is1"
  }
}
if ($VersionArm64 -eq $LatestVersion) {
  ...
  $this.CurrentState.Installer += [ordered]@{
    ...
    ProductCode     = "ImageMagick $($LatestVersion.Split('-')[0]) Q16 (arm64)_is1"
  }
}
