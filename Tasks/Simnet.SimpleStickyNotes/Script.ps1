...
function Get-ReleaseNotes {
  try {
    $Object3 = Invoke-WebRequest -Uri 'https://www.simplestickynotes.com/simple-sticky-notes-version-history' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object3.SelectSingleNode("//strong[contains(text(), '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesTitleNode) {
      $ReleaseNotesNodes = for ($Node = $ReleaseNotesTitleNode.NextSibling; $Node -and $Node.Name -ne 'strong'; $Node = $Node.NextSibling) {
...
$Object1 = Invoke-WebRequest -Uri 'https://www.simplestickynotes.com/download-start'
