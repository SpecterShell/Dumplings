...
$this.CurrentState.Installer += [ordered]@{
  ...
  InstallerUrl  = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.Contains('x64') -and $_.name -notmatch 'installer' }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
# $this.CurrentState.Installer += [ordered]@{
#   Architecture  = 'x64'
#   InstallerType = 'wix'
