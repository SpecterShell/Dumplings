...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['ZWCAD2026'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['ZWCAD2026'] = $OldReleases = [ordered]@{}
}
