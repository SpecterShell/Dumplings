function Read-Installer {
  ...
  $InstallerFileExtracted = Expand-TempArchive -Path $InstallerFile -RelativeFilePath 'ADPhotoEditFreeSetup.exe' -CollisionAction Rename
  try {
    # Version
...
function Get-ReleaseNotes {
  try {
    $Object3 = Invoke-RestMethod -Uri 'https://cjwdev.com/Software/ADPhotoEdit/LatestVersionFree.xml'
    ...
    if (($Object3.UpdateInformation.VersionString -replace '(\.0+)+$') -eq ($this.CurrentState.Version -replace '(\.0+)+$')) {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotes'
        Value  = $Object3.UpdateInformation.NewFeatures.string | Format-Text
      }
    } else {
...
$Prefix = 'https://cjwdev.com/Software/ADPhotoEdit/Download.html'
