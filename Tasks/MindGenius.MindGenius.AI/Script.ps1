function Read-Installer {
  foreach ($Installer in $this.CurrentState.Installer) {
    ...
    $InstallerInfo = Get-InstallShieldMsiInfo -Path $InstallerFile -Name 'MindGenius AI.msi'
    # Version
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Global:DumplingsStorage.MindGeniusDownloadPage.Links.Where({ try { $_.href -match 'MindGeniusAI\.exe$' } catch {} }, 'First')[0].href
}
