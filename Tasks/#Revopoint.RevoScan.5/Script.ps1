...
if (Test-Path -Path $OldReleasesPath) {
  $Global:DumplingsStorage['RevoScan5'] = $OldReleases = Get-Content -Path $OldReleasesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $Global:DumplingsStorage['RevoScan5'] = $OldReleases = [ordered]@{}
}
