function Read-Installer {
  ...
  # Version
  $this.CurrentState.Version = $InstallerFile | Read-ProductVersionFromInno
  # InstallerSha256
  $this.CurrentState.Installer[0]['InstallerSha256'] = (Get-FileHash -Path $InstallerFile -Algorithm SHA256).Hash
}

$Prefix = 'https://tmog.org/'
$Object1 = Invoke-WebRequest -Uri $Prefix

# Installer
$this.CurrentState.Installer += [ordered]@{
  InstallerType = 'inno'
  InstallerUrl  = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href.Contains('TMOG') } catch {} }, 'First')[0].href
}
