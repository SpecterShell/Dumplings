function Read-Installer {
  ...
  # ProductCode
  $this.CurrentState.Installer[0]['ProductCode'] = "Crucial Storage Executive $($this.CurrentState.Version)"
  # NestedInstallerFiles
  $this.CurrentState.Installer[0]['NestedInstallerFiles'] = @(
    [ordered]@{
      RelativeFilePath = [System.IO.Path]::GetRelativePath($InstallerFileExtracted, $InstallerFile2.FullName)
    }
  )
