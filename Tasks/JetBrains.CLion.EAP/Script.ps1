$Object1 = $Global:DumplingsStorage.JetBrainsApps.CL.eap
...
$this.CurrentState.Installer += $InstallerX64 = [ordered]@{
  ...
  ProductCode            = "CLion $($Object1.build)"
  AppsAndFeaturesEntries = @(
    [ordered]@{
      DisplayName = "CLion $($Object1.build)"
      ProductCode = "CLion $($Object1.build)"
    }
  )
}
  ...
  'New|Changed|Updated' {
    try {
      ...
      if ($Object1.notesLink) {
        ...
        $this.CurrentState.Locale += [ordered]@{
          Key   = 'ReleaseNotesUrl'
          Value = 'https://www.jetbrains.com/clion/whatsnew/'
        }
        $this.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotesUrl'
          Value  = 'https://www.jetbrains.com/zh-cn/clion/whatsnew/'
        }
      }
    } catch {
      $_ | Out-Host
