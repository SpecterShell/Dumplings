...
$PatternFilename = 'CrystalDiskInfo\d+(?:_\d+)+[a-zA-Z]?Shizuku.*\.exe'
...
if ($Assets2 = $Object1.Where({ $_.title.'#cdata-section' -eq "${RootPath}/$($this.CurrentState.Version)/CrystalDiskInfo$($this.CurrentState.Version.Replace('.', '_'))Shizuku.exe" })) {
  $Assets = $Assets2
}
