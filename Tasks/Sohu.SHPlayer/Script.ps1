...
switch (Compare-State) {
  ({ $_ -ge 1 }) {
    ...
    try {
      if ($Object2.SelectSingleNode('/html/body/div[3]/div/div[1]/div[2]/div/p/span').InnerText.Contains($Task.CurrentState.Version)) {
        # ReleaseTime
        ...
        $Task.CurrentState.Locale += [ordered]@{
          Locale = 'zh-CN'
          Key    = 'ReleaseNotes'
          Value  = $Object2.SelectNodes('/html/body/div[3]/div/div[1]/div[2]/div/div') | Get-TextContent | Format-Text
        }
      } else {
