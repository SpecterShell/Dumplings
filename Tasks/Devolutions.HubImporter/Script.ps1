# Version
$this.CurrentState.Version = $Global:DumplingsStorage.DevolutionsApps._.'HubImporter.Version'

# Installer
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = $Global:DumplingsStorage.DevolutionsApps._.'HubImporter.Url' -replace '\.exe$', '.msi'
}
  ...
  'New|Changed|Updated' {
    try {
      ...
      $this.CurrentState.Locale += [ordered]@{
        Key   = 'ReleaseNotesUrl'
        Value = $ReleaseNotesUrl = 'https://devolutions.net/password-hub-importer/release-notes/'
      }

      $Object2 = Invoke-WebRequest -Uri $ReleaseNotesUrl | ConvertFrom-Html

      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//h4[@id='v$($this.CurrentState.Version)']")
