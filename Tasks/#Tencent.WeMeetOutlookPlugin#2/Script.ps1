...
if (Test-Path -Path $OldReleaseNotesPath) {
  $LocalStorage['WeMeetOutlookPlugin2'] = $OldReleaseNotes = Get-Content -Path $OldReleaseNotesPath -Raw | ConvertFrom-Yaml -Ordered
} else {
  $LocalStorage['WeMeetOutlookPlugin2'] = $OldReleaseNotes = [ordered]@{}
}
