...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://www.digitalvolcano.co.uk/tcchangelog.html' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//strong[contains(text(), '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesTitleNode) {
      if (($ReleaseTimeNode = $ReleaseNotesTitleNode.NextSibling).InnerText -match '(\d+\W+[a-zA-Z]+\W+20\d{2})') {
        # ReleaseTime
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://www.digitalvolcano.co.uk/download/TextCrawlerPro_setup.exe'
}
