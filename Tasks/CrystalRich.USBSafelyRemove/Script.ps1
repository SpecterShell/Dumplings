...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Get-RedirectedUrl -Uri 'https://safelyremove.com/startdownload.htm?imm'
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      ...
      $Object2 = (Invoke-RestMethod -Uri 'https://blog.crystalrich.com/category/usbsr/feed/').Where({ $_.title.Contains($this.CurrentState.Version.Split('.')[0..1] -join '.') -and -not $_.title.Contains('beta') }, 'First')
