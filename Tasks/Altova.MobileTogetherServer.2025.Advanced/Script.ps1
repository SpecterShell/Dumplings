$Object1 = Invoke-WebRequest -Uri 'https://www.altova.com/thankyou?ProductCode=TS&EditionCode=A&InstallerType=Product&Lang=multi&OperatingSystem=win32'
...
# Version
$this.CurrentState.Version = [regex]::Match($this.CurrentState.Installer[0].InstallerUrl, '/MT_(\d+(?:\.\d+)+)/').Groups[1].Value
  ...
  'New|Changed|Updated' {
    ...
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.altova.com/releasenotes/getnotes' -Method Post -Body @{
        category = 'mobile'
        product  = 'MobileTogetherServer'
        version  = "mt$($this.CurrentState.Version)"
      } | ConvertFrom-Html
