$Object1 = Invoke-GitHubApi -Uri 'https://api.github.com/repos/l0ng-ai/tty7/releases/latest'
    ...
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node; $Node = $Node.NextSibling) {
          if ($Node.Name -eq 'table' -and $Node.InnerText -match 'Architecture') { $Node = $Node.NextSibling; if ($Node) { $Node } }
          else { $Node }
        }
        # ReleaseNotes (en-US)
