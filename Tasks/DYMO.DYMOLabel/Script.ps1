$Object1 = $Global:DumplingsStorage.DYMOApps.'DYMO Softwares'.'DYMO Label Software'.Windows.GetEnumerator() | Sort-Object -Property { [regex]::Match($_.Name, 'v(\d+(\.\d+)+)').Groups[1].Value -creplace '\d+', { $_.Value.PadLeft(20) } } -Bottom 1
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    foreach ($Installer in $this.CurrentState.Installer) {
      ...
      $InstallerFile2 = Join-Path $InstallerFileExtracted 'DYMO Label.msi'
      # RealVersion
