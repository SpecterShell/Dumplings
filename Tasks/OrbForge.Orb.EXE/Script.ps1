...
function Get-ReleaseNotes {
  try {
    $Object1 = Invoke-WebRequest -Uri 'https://orb.net/the-forge/changelog' | ConvertFrom-Html

    $ReleaseNotesNode = $Object1.SelectSingleNode("//div[contains(@class, 'changelog-item') and contains(.//div[contains(@class, 'version-col')], '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesNode) {
      # ReleaseNotes (en-US)
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://pkgs.orb.net/earlyaccess/windows/Orb-installer.exe'
}
