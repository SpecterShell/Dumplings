function Read-Installer {
  foreach ($Installer in $this.CurrentState.Installer) {
    ...
    $InstallerFile2 = Join-Path $InstallerFileExtracted 'WiFi Sensor Software.msi'
    # Version
...
$Prefix = 'https://lascarelectronics.com/intl/software/easylog-software/easylog-wifi/'
$Object1 = Invoke-WebRequest -Uri $Prefix

# Installer
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.EndsWith('.exe') } catch {} }, 'First')[0].href | ConvertTo-UnescapedUri | ConvertTo-Https
}
