$Object1 = Invoke-GitHubApi -Uri 'https://api.github.com/repos/yuanlang12/OpenPipal/releases/latest'
  ...
  'New|Changed|Updated' {
    try {
      ...
      if (-not [string]::IsNullOrWhiteSpace($Object1.body)) {
        ...
        $ReleaseNotesNodes = for ($Node = $ReleaseNotesObject.ChildNodes[0]; $Node -and -not ($Node.Name -in @('h1', 'h2') -and $Node.InnerText -match 'Install'); $Node = $Node.NextSibling) { $Node }
        # ReleaseNotes (en-US)
