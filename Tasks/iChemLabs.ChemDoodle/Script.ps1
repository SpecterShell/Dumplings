...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Global:DumplingsStorage.iChemLabsPrefix $Global:DumplingsStorage.iChemLabsApps.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href.Contains('ChemDoodle-') } catch {} }, 'First')[0].href
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.ichemlabs.com/support/version-history?product=ChemDoodle' | Read-ResponseContent | ConvertFrom-Html
