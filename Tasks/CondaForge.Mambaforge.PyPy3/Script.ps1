...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Object1.assets.Where({ $_.name.EndsWith('.exe') -and $_.name.StartsWith('Mambaforge') -and $_.name.Contains('x86_64') -and $_.name.Contains('pypy') }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    $this.CurrentState.Installer[0]['ProductCode'] = "Mambaforge-pypy3 $($this.CurrentState.Version) (Python ${PythonVersion} 64-bit)"
