...
$Object2 = $Object1.dlcDetailsView.downloadFile | Where-Object -FilterScript { $_.name.EndsWith('.msi') } | Sort-Object -Property { [ChunkVersion]([regex]::Match($_.name, '(\d+(?:\.\d+)+)').Groups[1].Value) } -Bottom 1
