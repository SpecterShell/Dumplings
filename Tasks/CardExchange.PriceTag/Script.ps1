$Object1 = $Global:DumplingsStorage.CardExchangeDownloadPage.SelectSingleNode('//tr[contains(@class, "fs-table-row") and contains(./td[contains(@class, "fs-table-title")]//div[contains(@class, "el-title")], "CardExchange® PriceTag")]')
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      ...
      $ReleaseNotesNode = $ReleaseNotesPage.SelectSingleNode("//div[contains(@id, 'module-') and contains(.//strong, 'CardExchange® PriceTag')]//div[contains(@class, 'fs-changelog') and contains(.//h3[contains(@class, 'el-version')], '$($this.CurrentState.RealVersion)')]")
      if ($ReleaseNotesNode) {
