...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://trypear.ai/changelog' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//h3[contains(., 'v$($this.CurrentState.Version)')]")
    if ($ReleaseNotesTitleNode) {
      $ReleaseNotesNodes = for ($Node = $ReleaseNotesTitleNode.NextSibling; $Node; $Node = $Node.NextSibling) {
        if (-not $Node.HasClass('lg:hidden')) {
          $Node
        }
      }
      # ReleaseNotes (en-US)
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://pearai-app.nyc3.digitaloceanspaces.com/PearAI-latest/windows-x64/PearAISetup.exe'
}
