...
$Object1 = Invoke-RestMethod -Uri 'https://www.high-logic.com/process.php' -Body @{
  ...
  tar = 'x86'
  app = 'sh'
  ver = $this.LastState.Contains('Version') ? $this.LastState.Version : '8.0.0.319'
  lid = '0'
  mod = '0'
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.high-logic.com/font-generator/scanahand/changelog' | ConvertFrom-Html

      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//li[@class='el-item' and contains(.//h3[contains(@class, 'el-title')], 'Scanahand $($this.CurrentState.Version)')]")
