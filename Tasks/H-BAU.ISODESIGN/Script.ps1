function Read-Installer {
  ...
  7z.exe e -aoa -ba -bd -y -o"${InstallerFileExtracted}" $InstallerFile 'IsoDesign.msi' | Out-Host
  $InstallerFile2 = Join-Path $InstallerFileExtracted 'IsoDesign.msi'
  # Version
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri 'https://pohlcon.com/fileadmin/crossbase/SOF/PRO/' ([uri]::EscapeDataString($Global:DumplingsStorage.PohlConApps.results.Where({ $_.cbid -eq 10799 }, 'First')[0].items.Where({ $_.sys_language_uid -eq 1 }, 'First')[0].file_metadata.fileName))
}
