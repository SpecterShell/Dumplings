...
$VersionX64 = $Object6.'installer-gui-script'.choice.'pkg-ref'.Where({ $_.id -eq 'Bonjour64' }, 'First')[0].version
...
$this.CurrentState.Installer += [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Object5.Packages.GetEnumerator().Where({ $_.URL.Contains('Bonjour64.msi') }, 'First')[0].URL | ConvertTo-Https
}
