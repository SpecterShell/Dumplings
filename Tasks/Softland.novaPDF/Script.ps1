...
function Get-ReleaseNotes {
  try {
    if ($Global:DumplingsStorage.Contains('novaPDF') -and $Global:DumplingsStorage.novaPDF.Contains($this.CurrentState.Version)) {
      # ReleaseTime
      $this.CurrentState.ReleaseTime = $Global:DumplingsStorage.novaPDF[$this.CurrentState.Version].ReleaseTime | Get-Date -AsUTC

      # ReleaseNotes (en-US)
      $this.CurrentState.Locale += [ordered]@{
        Locale = 'en-US'
        Key    = 'ReleaseNotes'
        Value  = $Global:DumplingsStorage.novaPDF[$this.CurrentState.Version].ReleaseNotes
      }
    } else {
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://download.novapdf.com/download/setup/novapdf-11.exe'
}
