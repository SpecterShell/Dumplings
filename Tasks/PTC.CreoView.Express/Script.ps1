...
$InstallerUrl = $Object1.SelectSingleNode('//primary-to-action[@text="Download"]').Attributes['url'].Value | Split-Uri -LeftPart Path
...
$this.CurrentState.Installer += [ordered]@{
  ...
  NestedInstallerFiles = @(
    [ordered]@{
      RelativeFilePath = "$($Object2.FileName | Split-Path -LeafBase).exe"
    }
  )
}
