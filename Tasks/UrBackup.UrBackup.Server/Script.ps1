...
$this.CurrentState.Installer += $InstallerEXE = [ordered]@{
  ...
  InstallerUrl  = 'https:' + $Object1.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href.Contains('Server') } catch {} }, 'First')[0].href | ConvertTo-UnescapedUri
}
$VersionEXE = [regex]::Match($InstallerEXE.InstallerUrl, '(\d+(\.\d+)+)').Groups[1].Value

$this.CurrentState.Installer += $InstallerMSI = [ordered]@{
  InstallerType = 'wix'
  InstallerUrl  = 'https:' + $Object1.Links.Where({ try { $_.href.EndsWith('.msi') -and $_.href.Contains('Server') } catch {} }, 'First')[0].href | ConvertTo-UnescapedUri
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.urbackup.org/server_changelog.html' | ConvertFrom-Html
