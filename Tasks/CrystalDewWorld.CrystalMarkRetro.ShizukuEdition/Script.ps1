...
$PatternFilename = 'CrystalMarkRetro\d+(?:_\d+)+[a-zA-Z]?Shizuku.*\.exe'
...
if ($Assets2 = $Object1.Where({ $_.title.'#cdata-section' -eq "${RootPath}/$($this.CurrentState.Version)/CrystalMarkRetro$($this.CurrentState.Version.Replace('.', '_'))Shizuku.exe" })) {
  $Assets = $Assets2
}
