...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    ...
    try {
      ...
      $ReleaseNotesTitleNode = $Object3.SelectSingleNode("//*[@id='goldview_history_tabpanel']//h2[contains(text(), 'Version $($this.CurrentState.Version.Split('.')[0..1] -join '.') Build $($this.CurrentState.Version.Split('.')[3])')]")
