...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      ...
      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//td[@id='postmessage_119526099']/text()[contains(., '$(Get-Date -Format 'yyyy.M.d' -AsUTC)') or contains(., '$('{0}.1.{2}.64' -f $this.CurrentState.Version.Split('.'))')]")
