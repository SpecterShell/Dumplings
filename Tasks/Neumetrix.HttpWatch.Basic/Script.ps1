...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://www.httpwatch.com/download/versionhistory/' | ConvertFrom-Html

    $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//p[@class='history' and contains(., '$($this.CurrentState.Version)')]")
    if ($ReleaseNotesTitleNode) {
      ...
      $ReleaseNotesNodes = for ($Node = $ReleaseNotesTitleNode.NextSibling; $Node -and -not ($Node.Name -eq 'a' -and $Node.Attributes.Contains('id')) -and -not ($Node.Name -eq 'p' -and $Node.HasClass('history')); $Node = $Node.NextSibling) { $Node }
      # ReleaseNotes (en-US)
...
$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = 'https://download.httpwatch.com/httpwatch.exe'
}
