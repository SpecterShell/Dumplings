...
function Get-ReleaseNotes {
  try {
    $Object2 = Invoke-WebRequest -Uri 'https://www.axis.com/ftp/pub_soft/cam_srv/optimizer_milestone/latest/relnote.txt'

    if ($Object2.Content -match "Software version:\s+$($this.CurrentState.Version)") {
      $Object3 = [System.IO.StreamReader]::new($Object2.RawContentStream)

      while (-not $Object3.EndOfStream) {
        $String = $Object3.ReadLine()
        if ($String -match '^Changes since version') {
      ...
      if (-not $Object3.EndOfStream) {
        ...
        while (-not $Object3.EndOfStream) {
          $String = $Object3.ReadLine()
          if ($String -notmatch '^Changes since version') {
...
$Prefix = 'https://www.axis.com/products/axis-optimizer-for-xprotectr'
$Object1 = Invoke-WebRequest -Uri $Prefix

$this.CurrentState.Installer += [ordered]@{
  InstallerUrl = Join-Uri $Prefix $Object1.Links.Where({ try { $_.href.EndsWith('.exe') -and $_.href -match 'AxisOptimizer' } catch {} }, 'First')[0].href
}
