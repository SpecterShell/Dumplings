$Object1 = (Invoke-GitHubApi -Uri "https://api.github.com/repos/brechtsanders/winlibs_mingw/releases").Where({ -not $_.prerelease -and $_.tag_name.Contains('mcf') -and $_.tag_name.Contains('ucrt') -and $_.tag_name -notmatch 'snapshot' }, 'First')[0]

$VersionMatches = [regex]::Match($Object1.tag_name, '(?<gcc>\d+(?:\.\d+)+)mcf(?:-(?<llvm>\d+(?:\.\d+)+))?-(?<mingw>\d+(?:\.\d+)+)-ucrt-r(?<release>\d+)')
...
$this.CurrentState.Installer += $InstallerX86 = [ordered]@{
  Architecture = 'x86'
  InstallerUrl = $Object1.assets.Where({ $_.name.EndsWith('.zip') -and $_.name -notmatch 'llvm' -and $_.name.Contains('i686') }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
$this.CurrentState.Installer += $InstallerX64 = [ordered]@{
  Architecture = 'x64'
  InstallerUrl = $Object1.assets.Where({ $_.name.EndsWith('.zip') -and $_.name -notmatch 'llvm' -and $_.name.Contains('x86_64') }, 'First')[0].browser_download_url | ConvertTo-UnescapedUri
}
