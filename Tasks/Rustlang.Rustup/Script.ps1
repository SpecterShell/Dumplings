  ...
  'New|Changed|Updated' {
    try {
      ...
      if ($ReleaseNotesTitleNode) {
        ...
        $this.CurrentState.Locale += [ordered]@{
          Key   = 'ReleaseNotesUrl'
          Value = 'https://github.com/rust-lang/rustup/blob/master/CHANGELOG.md#' + ($ReleaseNotesTitleNode.InnerText -creplace '[^a-zA-Z0-9\-\s]+', '' -creplace '\s+', '-').ToLower()
        }
      } else {
        ...
        $this.CurrentState.Locale += [ordered]@{
          Key   = 'ReleaseNotesUrl'
          Value = 'https://github.com/rust-lang/rustup/blob/master/CHANGELOG.md'
        }
      }
    } catch {
      $_ | Out-Host
