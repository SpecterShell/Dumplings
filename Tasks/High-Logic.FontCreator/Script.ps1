...
$Object1 = Invoke-RestMethod -Uri 'https://www.high-logic.com/process.php' -Body @{
  ...
  tar = 'x86'
  app = 'fc'
  ver = $this.LastState.Contains('Version') ? $this.LastState.Version : '15.0.0.1000'
  lid = '0'
  mod = '0'
}
...
switch -Regex ($this.Check()) {
  'New|Changed|Updated' {
    try {
      $Object2 = Invoke-WebRequest -Uri 'https://www.high-logic.com/font-editor/fontcreator/changelog' | ConvertFrom-Html

      $ReleaseNotesTitleNode = $Object2.SelectSingleNode("//li[@class='el-item' and contains(.//h3[contains(@class, 'el-title')], 'FontCreator $($this.CurrentState.Version)')]")
