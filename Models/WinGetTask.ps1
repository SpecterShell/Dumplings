...
class WinGetTask {
  ...
  [void] Submit() {
    if (-not $this.Preference.NoSubmit) {
      ...
      try {
        if (Test-Path Env:\GITHUB_RUN_ID) {
          $NewPRBody = "This pull request is automatically generated by [🥟 Dumplings](https://github.com/SpecterShell/Dumplings) in [#${Env:GITHUB_RUN_NUMBER}](https://github.com/${OriginOwner}/${OriginRepo}/actions/runs/${Env:GITHUB_RUN_ID})"
        } else {
